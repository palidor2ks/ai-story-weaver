...
**Owner decision (2026-06-13): keep inferred in denominator**
`evidence_type='inferred'` answers stay in the URL% denominator. They represent real answers that should eventually be replaced with properly-sourced ones. The metric stays red until those 204,506 answers get real citations. This is intentional — the red metric is a forcing function toward reanswering them with citations, not a reporting artifact to be hidden.

**What "reanswering inferred" actually means**
Inferred answers were generated by the populate-candidate-answers function based on roll-call votes — the answer_value is right but there's no source URL because the function didn't record which vote drove it. To get to 35%, we need to either:
1. Back-fill source URLs onto existing inferred answers (re-run enrichment with the vote-to-bill URL resolved), OR
2. Replace them with `evidence_type='sourced'` answers that have congress.gov URLs

The tier-2 vote citation pipeline (scripts/answers-enrichment/) already does (2) for new answers. The gap is the existing 204,506 inferred rows.

**Next**
Design a back-fill pass to resolve source URLs for existing `evidence_type='inferred'` answers: find the sponsorship/cosponsor bill record that originally drove the answer, look up its congress.gov URL, and set `source_url + evidence_type='sourced'`. This is pure SQL + the existing tier-2 pipeline logic — no distiller needed.
