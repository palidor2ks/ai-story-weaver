...
score is only corrected when their answers are regenerated. So
`supabase/migrations/20260620220000_score_sanity_sweeper.sql` automates exactly the manual loop
above, as a self-terminating **detect → queue → fix** system modelled on `requeue-stalled-research`:
