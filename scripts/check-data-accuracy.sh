...
STATE_ERRS=$(q "select coalesce((stat_value->'nj'->>'errors7d')::int,0)+coalesce((stat_value->'fl'->>'errors7d')::int,0)+coalesce((stat_value->'ny'->>'errors7d')::int,0)+coalesce((stat_value->'tx'->>'errors7d')::int,0)+coalesce((stat_value->'nc'->>'errors7d')::int,0) from admin_stats_cache where stat_key='state_finance_stats'")
if [ "${STATE_ERRS:-0}" -gt 0 ]; then
  err "state-finance: $STATE_ERRS sync errors across NJ/FL/NY/TX/NC this week"
else
  ok "state-finance: NJ/FL/NY/TX/NC syncing clean (0 errors this week)"
fi
