...
function electionMatchesUserContext(election: ElectionResponseRow, context: { state: string; district: string | null; city: string | null }): boolean {
...
async function fetchFEC(state: string, district: string | null): Promise<ElectionPayload[]> {
  ...
  for (const cycle of cycles) {
    ...
    for (const { office, data } of settled) {
      const officeLabel = office === 'H' ? `U.S. House ${state}-${normalizedDistrict ?? ''}` : office === 'S' ? `U.S. Senate (${state})` : 'President of the United States';
      ...
      results.push({
        ...
        jurisdiction: office === 'H' ? districtJurisdiction(state, normalizedDistrict) : (office === 'S' ? state : 'US'),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!force) {
      ...
      const expectedJurisdictions = new Set<string>(['US', state.toUpperCase()]);
