...
-- Wrapped in exception handling: this is a data-only migration that relies on
-- columns (local_loans, etc.) added to production outside the migration chain.
-- On fresh preview databases the schema may be incomplete; skip gracefully.
DO $$
BEGIN
  WITH t AS (
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Skipping Moody finance_reconciliation seed on preview branch: %', SQLERRM;
END $$;
