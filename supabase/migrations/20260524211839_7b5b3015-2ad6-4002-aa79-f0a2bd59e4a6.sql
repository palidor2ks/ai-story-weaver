...
DECLARE
  ...
  v_refund_rx text;
BEGIN
  -- Build a single combined regex of active vendor-refund patterns (uppercased).
  SELECT string_agg(regexp_replace(upper(name), '([.\\+*?()\[\]{}|^$])', '\\\1', 'g'), '|')
    INTO v_refund_rx
  ...
  IF NOT v_all_cycles THEN
    ...
    WITH filtered AS (
      ...
      WHERE dcmv.cycle = p_cycle
        ...
        AND (v_refund_rx IS NULL OR upper(dcmv.display_name) !~ v_refund_rx)
    ), counted AS (
  ...
  WITH filtered AS (
    ...
    WHERE (p_type IS NULL OR p_type = '' OR p_type = 'all' OR p_type = ANY(m.types::text[]))
      ...
      AND (v_refund_rx IS NULL OR upper(m.display_name) !~ v_refund_rx)
  ), counted AS (
    SELECT count(*)::bigint AS total_count FROM filtered
  )
