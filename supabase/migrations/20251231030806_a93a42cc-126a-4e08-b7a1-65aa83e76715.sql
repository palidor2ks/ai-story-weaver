-- Update get_contribution_totals to also exclude Line 11D (candidate contributions) from itemized_total
...
AS $function$
  SELECT
    ...
    -- Exclude Line 15 (other receipts), Line 13A (candidate loans), Line 11D (candidate contributions), and memo_code='X' duplicates
    COALESCE(SUM(CASE 
      WHEN (memo_code IS NULL OR memo_code != 'X')
        AND (line_number IS NULL OR (line_number NOT LIKE '15%' AND line_number NOT IN ('13A', '11D')))
      THEN amount ELSE 0 
...
-- Update get_contribution_totals_by_committee to also exclude Line 11D from itemized_total
