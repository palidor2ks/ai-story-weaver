-- Add `today` (the DB's current date) to the caption-facts payload so the social
-- caption composer can frame tense relative to the cycle's election (an upcoming
-- race must not be described in the past tense).
