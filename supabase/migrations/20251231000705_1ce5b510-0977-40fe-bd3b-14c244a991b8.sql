...
-- Recreate get_contribution_totals with Line 15 exclusion and other_total column
CREATE FUNCTION public.get_contribution_totals(p_candidate_id text, p_cycle text)
 RETURNS TABLE(individual_total bigint, pac_total bigint, party_total bigint, itemized_total bigint, transfers_total bigint, earmarked_total bigint, passthrough_total bigint, other_total bigint, contribution_count bigint)
 LANGUAGE sql
...
AS $function$
  SELECT
    ...
    -- Exclude Line 15 (Other Receipts) from itemized total
    COALESCE(SUM(CASE 
      WHEN (memo_code IS NULL OR memo_code != 'X')
        AND (line_number IS NULL OR line_number NOT LIKE '15%')
      THEN amount ELSE 0 
    ...
    -- Track Line 15 (Other Receipts) separately
    COALESCE(SUM(CASE 
...
-- Recreate get_contribution_totals_by_committee with same changes
CREATE FUNCTION public.get_contribution_totals_by_committee(p_candidate_id text, p_cycle text)
 RETURNS TABLE(committee_id text, individual_total bigint, pac_total bigint, party_total bigint, itemized_total bigint, transfers_total bigint, earmarked_total bigint, other_total bigint, contribution_count bigint)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
