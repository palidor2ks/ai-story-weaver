-- Add new columns to finance_reconciliation for gross individual tracking
ALTER TABLE finance_reconciliation 
ADD COLUMN IF NOT EXISTS local_gross_individual bigint,
ADD COLUMN IF NOT EXISTS memo_x_amount bigint;
...
-- Recreate get_contribution_totals with gross_individual_total
...
-- Recreate get_contribution_totals_by_committee with gross_individual_total
CREATE FUNCTION public.get_contribution_totals_by_committee(p_candidate_id text, p_cycle text)
 RETURNS TABLE(committee_id text, individual_total bigint, pac_total bigint, party_total bigint, itemized_total bigint, transfers_total bigint, earmarked_total bigint, other_total bigint, loans_total bigint, organization_total bigint, contribution_count bigint, gross_individual_total bigint)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
