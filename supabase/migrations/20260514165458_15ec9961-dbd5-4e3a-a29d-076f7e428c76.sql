-- Fix inflated all-cycles donor totals: aggregations were Cartesian-multiplied by name_variations × types unnests
...
WITH sums AS (
  ...
  GROUP BY display_name, type
),
primary_ids AS (
  SELECT DISTINCT ON (display_name, type)
    display_name, type, primary_id
  FROM private.donor_consolidated_mv
  ORDER BY display_name, type, total_amount DESC NULLS LAST
),
names AS (
  SELECT display_name, type, array_remove(array_agg(DISTINCT nv), NULL) AS name_variations
  FROM private.donor_consolidated_mv m
  LEFT JOIN LATERAL unnest(m.name_variations) AS nv ON true
  GROUP BY display_name, type
),
types_agg AS (
  SELECT display_name, type, array_remove(array_agg(DISTINCT t), NULL) AS types_text
  FROM private.donor_consolidated_mv m
...
SELECT
  ...
  s.search_text
FROM sums s
JOIN primary_ids p USING (display_name, type)
JOIN names n       USING (display_name, type)
JOIN types_agg ta  USING (display_name, type);
