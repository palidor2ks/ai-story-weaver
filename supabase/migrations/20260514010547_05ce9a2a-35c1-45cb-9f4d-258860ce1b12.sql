...
create materialized view public.donor_consolidated_mv as
select
...
create unique index donor_consolidated_mv_row_id_idx
  on public.donor_consolidated_mv (row_id);

create index donor_consolidated_mv_total_amount_idx
  on public.donor_consolidated_mv (total_amount desc nulls last);

create index donor_consolidated_mv_cycle_amount_idx
  on public.donor_consolidated_mv (cycle, total_amount desc nulls last);

create index donor_consolidated_mv_display_name_idx
  on public.donor_consolidated_mv (display_name);

create index donor_consolidated_mv_cycle_display_name_idx
  on public.donor_consolidated_mv (cycle, display_name);

create index donor_consolidated_mv_types_idx
  on public.donor_consolidated_mv using gin (types);

create index donor_consolidated_mv_search_text_trgm_idx
  on public.donor_consolidated_mv using gin (search_text gin_trgm_ops);

grant select on public.donor_consolidated_mv to authenticated, service_role;
revoke all on public.donor_consolidated_mv from anon;
...
as $$
  ...
  from public.donor_consolidated_mv dcmv
