...
-- Add pass_through_excluded column to finance_reconciliation if not exists
ALTER TABLE finance_reconciliation 
ADD COLUMN IF NOT EXISTS pass_through_excluded numeric DEFAULT 0;

-- Recreate get_contribution_totals with pass-through detection
  ...
  SELECT
    ...
    COALESCE(SUM(CASE 
      WHEN c.contributor_type = 'Individual' 
        ...
        AND NOT (c.memo_text ILIKE '%NOTE: ABOVE CONTRIBUTION%')
      THEN c.amount ELSE 0 
    END), 0)::numeric as individual_total,
    
    -- Organization contributions (Line 11AI, not memo_code='X', NOT a conduit, not pass-through)
    COALESCE(SUM(CASE 
    ...
    -- PAC contributions (Line 11C, not pass-through)
    COALESCE(SUM(CASE 
    ...
    COALESCE(SUM(CASE 
      WHEN c.memo_text ILIKE '%NOTE: ABOVE CONTRIBUTION%'
        AND COALESCE(c.memo_code, '') != 'X'
...
-- Recreate get_contribution_totals_by_committee with same pass-through detection
