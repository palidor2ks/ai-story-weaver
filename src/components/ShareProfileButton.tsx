...
import { useShareProfileSpenderCauses } from '@/hooks/useShareProfileData';
...
export const ShareProfileButton = ({
  ...
  const { data: spenderCauseMap } = useShareProfileSpenderCauses(topSpenderIds);
  ...
  const { data: aiAnalysis, isLoading: aiAnalysisLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await invokeEdgeFunction('ai-recipient-analysis', {
        body: {
  ...
  const { data: headlineCaption } = useQuery({
    ...
    staleTime: 1000 * 60 * 10,
    queryFn: async () => {
      const { data, error } = await invokeEdgeFunction('compose-candidate-caption', {
