                          ...
                          <TableCell>
                            {currentAlias ? (
                              ...
                              <DirectDonorCauseCell
                                value={directCauseByDonorKey.get(key)?.primary_cause_id || ''}
                                source={directCauseByDonorKey.get(key)?.assigned_by || null}
                                causes={causes}
                                onAssign={(causeId) =>
...
function DirectDonorCauseCell({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {source && (
        <Badge variant="outline" className="text-[10px]">{source === 'admin' ? 'Admin' : source}</Badge>
      )}
    </div>
  );
}
