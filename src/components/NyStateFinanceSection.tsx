import {
  useNyLegislatorFinance,
  isNyStateLegislator,
  type NyTopContributor,
} from '@/hooks/useNyLegislatorFinance';
...
const isIndividual = (c: NyTopContributor) => c.is_individual;

/**
 * Campaign-finance section for NY state legislators, shown on the unified
 * candidate profile. Sourced from the NY State Board of Elections (data the FEC
 * doesn't cover). Renders nothing for non-NY-legislators or before contribution
 * data has synced.
 */
export function NyStateFinanceSection(props: Props) {
  const { data, isLoading } = useNyLegislatorFinance(props);

  if (!isNyStateLegislator(props)) return null;
    ...
    <Card className="mb-6 border-border/60">
      ...
      <CardContent className="space-y-5">
            ...
            {cycles.map((e) => (
              <div key={e.entity_s} className="flex items-center justify-between gap-3 text-sm">
                <span className="min-w-0 truncate text-muted-foreground">
                  ...
                  {e.election_type ? ` · ${e.election_type}` : ''}
                </span>
        ...
        <p className="pt-1 text-xs text-muted-foreground">
          Source:{' '}
          <a
            href="https://data.ny.gov/Government-Finance/Campaign-Finance-Disclosure-Reports-Data-Beginning/e9ss-239a"
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-0.5 underline hover:text-foreground"
          >
            NY State Board of Elections <ExternalLink className="h-3 w-3" />
          </a>
